...
const { JWT_SECRET, GOOGLE_ID, GOOGLE_SECRET, FACEBOOK_ID, FACEBOOK_SECRET } = require('../configs/config');
		...
		async (accessToken, refreshToken, profile, done) => {
			try {
				...
				if (user) {
					...
					let user = await User.findOne({ email: profile.emails[0].value }).populate({
						path: 'image',
						select: 'public_url'
					});
					if (user) {
		...
		async (accessToken, refreshToken, profile, done) => {
			try {
				...
				if (user) {
					...
					const user = await User.findOne({ email: profile.emails[0].value }).populate({
						path: 'image',
						select: 'public_url'
					});
					if (user) {
